...
original_author="chalosalvador"
original_name="flask_test"
original_urlname="flask-test"
original_description="Awesome flask_test created by chalosalvador" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_test "${name}"
...
then
    ...
    echo "then run 'flask_test' or 'python -m flask_test'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_test' or 'python -m flask_test' or 'poetry run flask_test'"
fi
