...
- 🎯 Entry points to execute your program using `python -m <flask_test>` or `$ flask_test` with basic CLI argument parsing.
...
---
# flask_test Flask Application

Awesome flask_test created by chalosalvador
...
```bash
git clone https://github.com/chalosalvador/flask-test flask_test
cd flask_test
make install
```
...
```bash
flask_test create-db   # run once
flask_test populate-db  # run once (optional)
flask_test add-user -u admin -p 1234  # ads a user
flask_test run
```
